...
pollDp$peso<-na.approx(pollDp$peso, na.rm = FALSE) #linearly interpolate NA values (weekends) for FX data in roder to merge w/poll data*
...
pollmeanT<-data.frame(pollDp[,c(1,2,7)])
colnames(pollmeanT) <- c("end_date","Disapprove","TCN")
pollmeanT$TCN<-as.numeric(pollmeanT$TCN)
dateN<-"2017-01-22"
